...
struct HelpView: View {
    ...
    private func chromeExtensionSection() -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Chrome handles link clicks internally, so macOS default browser settings alone cannot redirect links clicked within Chrome to LinkPrism.")
                .font(.body)
            ...
            Text("The Chrome extension intercepts links clicked within Chrome and forwards them to LinkPrism via the linkprism:// scheme.")
                .font(.body)

            Text("linkprism://route?url=<encoded_url>")
                .font(.system(.body, design: .monospaced))
                .padding(8)
                .background(Color.secondary.opacity(0.1))
                .cornerRadius(4)
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("3. Click \"Load unpacked\".")
                    .font(.body)
                Text("4. Select the LinkPrism extension folder.")
                    .font(.body)
            }
        }
    }
