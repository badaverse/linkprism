<div align="center">
  <img src="assets/app-icon.png" width="160" height="160" alt="ProfilePrism 아이콘">
  <h1>ProfilePrism</h1>
...
[**GitHub Releases**](https://github.com/badaverse/profileprism/releases/latest)에서 최신 `.dmg`를 다운로드하세요.
...
- **리브랜딩** — ProfileRouter에서 ProfilePrism으로 이름 변경
...
| 구성 요소 | 역할 |
|-----------|------|
| **ProfilePrism.app** | macOS 메뉴바 앱. 기본 브라우저로서 `http`/`https` URL을 가로채고 규칙에 따라 올바른 Chrome 프로필로 라우팅합니다. |
...
- **URL 선택 기억** — "이 URL은 다시 묻지 않기"를 체크하면 ProfilePrism이 선택을 기억
...
| 빌드 | Xcode 15+ |
| 설정 파일 | `~/Library/Application Support/ProfilePrism/rules.json` |
...
```
ProfilePrism/
├── ProfilePrism/                  # macOS SwiftUI 앱
│   ├── App/                       #   ProfilePrismApp, AppDelegate, URL 처리
...
├── ProfilePrismExtension/         # Chrome 확장 (Manifest V3)
...
ProfilePrism은 Xcode 문자열 카탈로그를 통해 다국어를 지원합니다. 새 언어를 추가하려면:

1. Xcode에서 `ProfilePrism/ProfilePrism/Localizable.xcstrings` 열기
2. 언어를 추가하고 번역 제공
3. Chrome 확장의 경우 `ProfilePrismExtension/_locales/` 아래에 로케일 폴더 추가

알려진 이슈와 기능 요청은 [Issues 페이지](https://github.com/badaverse/profileprism/issues)를 참고하세요.
